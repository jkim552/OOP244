...
2020/08/04  Fixed minor comparison error in comparing vehicle and const char*
-----------------------------------------------------------*/
...
namespace sdds {
	...
	bool operator==(const Vehicle& ve, const char* num) {
		...
		if ((strncmp(num2, ve.getLicensePlate(), strlen(num)) == 0) && strlen(ve.getLicensePlate()) == strlen(num)) {
			val = true;
		}
		return val;
	}
	...
	istream& Vehicle::read(istream& is) {
		...
		// Not comma seperated mode
			...
			do {
				...
				else if (tempPlate.length() > 8 || tempPlate.length() < 2) {
